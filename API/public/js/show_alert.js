...
let currentPage = 1;
let itemsPerPage = 5; // Số thông báo trên mỗi trang
let totalPages = 1;
let allAlerts = []; // Lưu toàn bộ dữ liệu để lọc và phân trang
let filteredAlerts = []; // Lưu dữ liệu sau khi lọc
let trashedAlerts = []; // Lưu thông báo đã xóa (đưa vào thùng rác)
// Enum định nghĩa các loại cảm biến
var SensorType;
(function (SensorType) {
    SensorType["ALL"] = "all";
    SensorType["LIGHT"] = "bh1750";
    SensorType["TEMPERATURE_HUMIDITY"] = "dht22";
...
function determineAlertIcon(alertType) {
    switch (alertType) {
        case "alert":
...
function createAlertHTML(alert) {
    ...
    return `
        <div class="notification-item unread ${alertType}" data-alert-id="${alert.id}">
            <div class="notification-icon">
                ${alertIcon}
            </div>
            <div class="notification-content">
                <div class="notification-title">${alert.sensor_type === "bh1750"
        ? "Cảnh báo ánh sáng"
        : alert.sensor_type === "dht22"
            ? "Cảnh báo nhiệt độ/độ ẩm"
            : alert.sensor_type === "soil_moisture"
                ? "Cảnh báo độ ẩm đất"
                : "Cảnh báo hệ thống"}</div>
...
function updateUnreadCount(alerts) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (trashedAlertsFromStorage) {
                ...
                console.log("Đã khôi phục từ localStorage:", trashedAlerts.length, "thông báo");
            }
            const response = yield fetch("http://localhost:3000/api/alert_reading");
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const alertData = yield response.json();
            console.log("Tổng số thông báo từ API:", alertData.data ? alertData.data.length : 0);
            ...
            console.log("Số thông báo còn lại sau khi loại bỏ thùng rác:", allAlerts.length);
...
function filterAlertsBySensorType(sensorType) {
    ...
    if (sensorType === SensorType.ALL) {
        ...
        filteredAlerts = allAlerts.filter((alert) => alert.sensor_type === sensorType);
    }
    displayAlerts(filteredAlerts);
    createPagination();
}
// Hàm để tìm thông báo theo ID trong mảng
function findAlertById(id, alertArray) {
    return alertArray.find((alert) => alert.id === id);
}
// Hàm để đưa thông báo vào thùng rác
function moveToTrash(alertId) {
    ...
    if (alertToTrash) {
        ...
        const notificationItem = document.querySelector(`.notification-item[data-alert-id="${alertId}"]`);
        if (notificationItem) {
...
function showTrashNotification() {
        ...
        button.addEventListener("click", function () {
            ...
            if (notificationItem) {
                if (this.querySelector(".fa-check")) {
                    ...
                    const alertId = parseInt(notificationItem.getAttribute("data-alert-id") || "0");
                    if (alertId) {
                        // Đưa vào thùng rác
                        moveToTrash(alertId);
                    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.location.pathname.includes("notifications.html")) {
        ...
        for (let i = 1; i < filterButtons.length; i++) {
            if (((_a = filterButtons[i].textContent) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase()) !== "tất cả") {
